    ...
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            ...
            if (target_price < current_price < target_price*1.1) and (ma5 < current_price):
                ...
                if krw > 5000:
                    ...
                    if count == 1:
                        deposit_buy = krw
                        count = 0
                    buy_result = upbit.buy_limit_order("KRW-BTC", int(target_price*0.98), krw/(int(target_price*0.982)))
            ...
            if btc > 0.00008:
                ...
                post_message(myToken,"#crypto", "일일 수익금 : " + float(deposit_sell-deposit_buy))
                post_message(myToken,"#crypto", "일일 수익율 : " + float((deposit_sell-deposit_buy)/deposit_buy))
                while order_count < 4:  # 미체결된 매수주문 취소
                    order=upbit.get_order("KRW-BTC",'wait','normal')[0]
